...
public class SingleFingerJNDTesterController extends PApplet {

	SingleFingerJNDTesterScreen singleFingerjndTesterScreen;
	Button quit;
	Recorder rec;
	...
	SingleFingerJNDTesterController (Arduinos ard){
		String[] a = {""};
		PApplet.runSketch(a, this);
		//this.averagesForJNDTest = b;
		singleFingerjndTesterScreen = new SingleFingerJNDTesterScreen(ard);		
	  ...
	  public void draw() {
		  ...
		  rec.update(singleFingerjndTesterScreen.index,millis(),singleFingerjndTesterScreen.arduino.previousA);
		  sampleCount=0;
		  }
	 ...
	 private void newPosOutput(){
	  ...
	  private void setHapticResponce(){
		  singleFingerjndTesterScreen.setHapticResponce(testsValues[currentTest][0],testsValues[currentTest][1],testsValues[currentTest][2]);	  
	  }
	  
	  private void getTouchData(){
		  testsValues[currentTest][4] = testsValues[currentTest][4]+singleFingerjndTesterScreen.left.getTouchCount();
		  testsValues[currentTest][5] = testsValues[currentTest][5]+singleFingerjndTesterScreen.right.getTouchCount();
	  }
	  
	  private void resetTouchData(){
		  singleFingerjndTesterScreen.left.resetTouchCount();
		  singleFingerjndTesterScreen.right.resetTouchCount();
	  }
	  ...
	  public void running(boolean g){
		  ...
		  singleFingerjndTesterScreen.running(false);
		  } else {
			  this.loop();
			  surface.setVisible(true);
			  singleFingerjndTesterScreen.running(true);
		  }
	  }
